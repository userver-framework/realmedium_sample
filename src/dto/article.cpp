...
Article Article::Parse(const models::FullArticleInfo& model,
  ...
  if (!model.tags.empty())
    article.tags =
        std::vector<std::string>(model.tags.begin(), model.tags.end());
  article.createdAt = model.createdAt;
  article.updatedAt = model.updatedAt;
  article.favoritesCount = model.articleFavoritedByUsernames.size();
  article.isFavorited =
      authUserId ? model.articleFavoritedByUserIds.find(authUserId.value()) !=
                       model.articleFavoritedByUserIds.end()
                 : false;
