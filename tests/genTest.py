...
def genListOfRandStr(amount_of_keys, lenght=0, _randint=np.random.randint):
    ...
    if lenght == 0:
        while len(keys) < amount_of_keys:
            keys |= {
                ''.join([pickchar() for _ in range(_randint(12, 20))])
                for _ in range(amount_of_keys - len(keys))
            }
    else:
        while len(keys) < amount_of_keys:
            keys |= {
                ''.join(
                    [pickchar() for _ in range(_randint(lenght - 2, lenght))],
                )
                for _ in range(amount_of_keys - len(keys))
            }
...
def fillUsers(con):
    ...
    usernames = USERS_DEFAULT + genUsername(
        max(0, COUNT_OF_USERS - lenUsersDefault),
    )
    imageList = [
        'https://fikiwiki.com/uploads/posts/2022-02/'
        '1644991237_23-fikiwiki-com-p-kartinki-krasivikh-koshechek-31.jpg',
        'https://webpulse.imgsmail.ru/imgpreview?key=pulse_cabinet'
        '-image-c52e6c20-071b-46fe-b0c0-cbad32ffb447&mb=webpulse',
        'https://vplate.ru/images/article/orig/2019/04/belye-koshki-s-golubymi'
        '-glazami-harakterna-li-dlya-nih-gluhota-i-kakimi-oni-byvayut-29.jpg',
        'https://limonos.ru/uploads/posts/2014-11/14148774331111.jpeg',
        'https://proprikol.ru/wp-content/uploads/2020/11/'
        'kartinki-paczanov-43.jpg',
    ]
    ...
    for i in range(COUNT_OF_USERS):
        ...
        dictOfExecute = {
            ...
            'password_hash': (
                '8d969eef6ecad3c29a3a629280e686'
                'cf0c3f5d5a86aff3ca12020c923adc6c92'
            ),
            'bio': randBio,
            'image': randImage,
        }
...
def fillArticleTag(con):
    ...
    for articleID in articleIDList:
        ...
        cnt += 1
        randTagIDList = random.sample(
            tagIDList, random.randint(1, len(tagIDList)),
        )
...
def fillComments(con):
    ...
    for i in range(COUNT_OF_COMMENTS):
        ...
        dictOfExecute = {
            'body': genText(random.randint(20, 60), 0),
            'user_id': userID,
...
def main():
    ...
    try:
        ...
        if COUNT_OF_USERS < MAX_FAVORITES:
            print(
                '\nERROR:\n\t COUNT_OF_USERS < MAX_FAVORITES: '
                f'{COUNT_OF_USERS} < {MAX_FAVORITES}',
            )
            exit(1)

        if COUNT_OF_USERS < MAX_FOLLOWERS:
            print(
                f'\nERROR:\n\t COUNT_OF_USERS < MAX_FOLLOWERS: '
                f'{COUNT_OF_USERS} < {MAX_FOLLOWERS}',
            )
            exit(1)
