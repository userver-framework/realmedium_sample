#include "articles_unfavorite.hpp"
...
userver::formats::json::Value Handler::HandleRequestJsonThrow(
    ...
    auto transaction = GetPg().Begin(
        "unfavorite_article_transaction",
        userver::storages::postgres::ClusterHostType::kMaster,
        userver::storages::postgres::Transaction::RW
    );

    auto res = transaction.Execute(sql::kDeleteFavoritePair.data(), user_id, slug);

    if (!res.IsEmpty()) {
        auto article_id = res.AsSingleRow<std::string>();

        transaction.Execute(sql::kDecrementFavoritesCount.data(), article_id);
        transaction.Commit();
    }
