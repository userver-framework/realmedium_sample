...
Article Article::Parse(const models::FullArticleInfo& model, std::optional<std::string> authUserId) {
    ...
    article.description = model.description;
    article.body = model.body;
    if (!model.tags.empty()) article.tags = std::vector<std::string>(model.tags.begin(), model.tags.end());
    article.createdAt = model.createdAt;
    article.updatedAt = model.updatedAt;
...
Serialize(const Article& article, userver::formats::serialize::To<userver::formats::json::Value>) {
    ...
    if (article.tags) {
        std::for_each(article.tags->begin(), article.tags->end(), [&builder](const auto& tag) {
            builder["tags"].PushBack(tag);
        });
    }
