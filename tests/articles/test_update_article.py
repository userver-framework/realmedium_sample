...
from endpoints import update_article
...
async def test_update_unknown_article(service_client):
    ...
    article = Article(Profile(user))
    response = await update_article(
        service_client, article, article.slug, get_user_token(response),
    )
...
async def test_invalid_access_update_article(service_client):
    ...
    article.change_fields()
    response = await update_article(
        service_client, article, slug, get_user_token(response),
    )
    assert response.status == HTTPStatus.NOT_FOUND
